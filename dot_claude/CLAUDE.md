...
- ファイル操作・検索にはBashではなく専用ツール（Read, Edit, Write, Glob, Grep）を使うこと
- Bashツールで `cd` を直接使用しないこと（`make` 等で間接的に呼ばれるのは可）
- サブディレクトリのスクリプト実行時はcdせず、ツールのオプションを使うこと
  （例: `pnpm --filter`, `npm --prefix`, Makefile経由 等）
